...
#define DJI_BAUD_RATE 115200 // Fixed
#define DJI_RX_PIN 0       // Brancher le fil Tx du GPS
#define DJI_TX_PIN 2       // pas utilisé
...
SoftwareSerial softSerial(DJI_RX_PIN, DJI_TX_PIN);
...
void setup() {
  ...
  softSerial.begin(DJI_BAUD_RATE);
...
void loop()
{
    ...
    case 0:
        ...
        if (decodedMessage != NAZA_MESSAGE_GPS) {
          if(millis() - gpsMap > 5000){
             Serial.println("No GPS data detected");
        ...
        if (NazaDecoder.getFixType() < 3){
            if (millis() - gpsMap > 1000) {
                Serial.print("Waiting... SAT=");  Serial.println(NazaDecoder.getNumSat());
            ...
            drone_idfr.set_current_position(NazaDecoder.getLat(), NazaDecoder.getLon(), NazaDecoder.getGpsAlt());
            drone_idfr.set_heading(NazaDecoder.getCog());
            drone_idfr.set_ground_speed(NazaDecoder.getSpeed());
            ...
            */
            ...
            /*if (millis() - gpsMap > 1000) {         
                Serial.print("LAT=");  Serial.print(NazaDecoder.getLat(), 6); Serial.print(" LONG="); Serial.print(NazaDecoder.getLon(), 6);
                Serial.print(" ALT=");  Serial.print(NazaDecoder.getGpsAlt());  Serial.print(" SAT=");  Serial.print(NazaDecoder.getNumSat());
                Serial.print(" DOP=");  Serial.print(NazaDecoder.getHdop());  Serial.print(" DISTfromLAST_POS_Sent=");  Serial.println(drone_idfr.get_distance_from_last_position_sent());
                gpsMap = millis();
            }*/
        }
        break;
